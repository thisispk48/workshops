...
4_2_pca_sklearn.py
==================
Production-Grade PCA Image Compression using Scikit-Learn.

Key Pedagogical Steps:
1. Load image as an H x W matrix X.
2. Initialize sklearn.decomposition.PCA(n_components=k, svd_solver='full').
3. Compress: Z = pca.fit_transform(X).
4. Reconstruct: X_rec = pca.inverse_transform(Z).
5. Extract explained_variance_ratio_ and evaluate storage compression savings.

Outputs:
- Individual reconstructed images: {output_dir}/monalisa_sklearn_k{k}.png
- Multi-panel comparison figure: {output_dir}/4_fig_pca_sklearn.png
- Interactive Plotly HTML: {output_dir}/4_fig_pca_sklearn.html
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Production-grade PCA image compression using Scikit-Learn."
    )
    parser.add_argument(
        "--image-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="full",
        choices=["full", "randomized", "auto"],
        help="SVD solver algorithm for PCA (default: 'full' for exact mathematical parity)",
    )
...
def pca_compress_sklearn(X, k_values, svd_solver="full"):
    H, W = X.shape
    total_pixels = H * W

    results = {}
    for k in k_values:
        # Step 1: Initialize Sklearn PCA
        pca = PCA(n_components=k, svd_solver=svd_solver)

        # Step 2: Fit and Transform (Compression)
        Z = pca.fit_transform(X)  # (H, k)

        # Step 3: Inverse Transform (Reconstruction)
        X_rec = pca.inverse_transform(Z)  # (H, W)
    ...
    # Full spectrum fit for elbow curve
    max_k = min(150, min(H, W))
    pca_full = PCA(n_components=max_k, svd_solver=svd_solver)
    pca_full.fit(X)
    cum_var_all = np.cumsum(pca_full.explained_variance_ratio_) * 100.0

    return results, cum_var_all


def create_matplotlib_figure(X, results, cum_var_all, k_values, svd_solver, output_path):
    H, W = X.shape
    total_pixels = H * W
    ...
    for idx, (k, coord) in enumerate(zip(k_values, grid_coords)):
        ...
        ax.set_title(
            f"Rank k = {k} (Scikit-Learn PCA, solver='{svd_solver}')\n"
    ...
    # 6: Cumulative Explained Variance Curve
    ...
    ax_elbow.plot(x_axis, cum_var_all[:n_comp_plot], color="#A855F7", linewidth=2.5, label="Sklearn Explained Var")
    ...
    ax_elbow.set_title("Information Retention Curve (Sklearn PCA)", fontsize=12, fontweight="bold", color="#F8FAFC")
    ...
    plt.suptitle(
        f"Production Scikit-Learn PCA Image Compression (svd_solver='{svd_solver}')",
        fontsize=16,
        fontweight="bold",
        color="#F8FAFC",
        y=0.98,
...
def create_plotly_html(X, results, cum_var_all, k_values, output_path):
    ...
    fig.update_layout(
        title="Interactive Production Scikit-Learn PCA Image Compression",
...
def main():
    ...
    print("=" * 70)
    print(f"Running Scikit-Learn PCA on: {args.image_path}")
    print(f"Component ranks (k): {k_values} | SVD Solver: {args.svd_solver}")
    print("=" * 70)

    X = load_image_as_matrix(args.image_path)
    H, W = X.shape

    results, cum_var_all = pca_compress_sklearn(X, k_values, svd_solver=args.svd_solver)

    # Save individual reconstructed images
    base_name = os.path.splitext(os.path.basename(args.image_path))[0]
    for k in k_values:
        rec_path = os.path.join(args.output_dir, f"{base_name}_sklearn_k{k}.png")
        Image.fromarray(results[k]["X_rec"].astype(np.uint8)).save(rec_path)
        print(f"Saved individual image: {rec_path}")

    # Save summary figures
    png_path = os.path.join(args.output_dir, "4_2_fig_pca_sklearn.png")
    html_path = os.path.join(args.output_dir, "4_2_fig_pca_sklearn.html")

    create_matplotlib_figure(X, results, cum_var_all, k_values, args.svd_solver, png_path)
